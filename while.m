...
|| Schleifen
|| LOOP-Schleife, Zählregisterinhalt nach Schleifenende unverändert!
...
|| WHILE-Schleife, Zählregisterinhalt nach Schleifenende 0
while :: adr -> [instruction] -> memory -> memory
while n fs ram = ram,                         if get n ram = 0
               = while n fs ((comp fs) ram),  otherwise
...
program is = program' (rep size 0,is)
             where
             program' (ram, [])   = ram
             program' (ram, f:fs) = program' (seq (force y) y, fs)
                                    where y = f ram
